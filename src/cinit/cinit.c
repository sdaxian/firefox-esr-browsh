...
typedef struct {
    ...
    // 不生成伪终端， service stdin stdout stderr 继承父进程，不使用日志线程
	// 不创建新的进程组
    bool no_pty;
...
static int load_service(const char *service)
{
    ...
    Try {
        ...
        load_value_as_bool("no_pty", &SRV(sid).no_pty);
...
static pid_t fork_and_exec(int service)
{
    ...
    if (SRV(service).no_pty) {
        ...
        p = forkpty(&pty_master, NULL, NULL, NULL);
    }
#else
    p = fork();
#endif
